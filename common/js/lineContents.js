...
var lineContents = function lineContents(wrap, type, data) {
    ...
    this.lineCon = function () {
        ...
        self.btnConfirm.off('click').on('click', function () {
            ...
            if (self.nlc.com() > 0) {
                ...
                console.log('좌변이 연결된 우변의 idx값 목록: ', self.nlc.connections);
    ...
    this.multiLineCon = function () {
        ...
        self.btnConfirm.off('click').on('click', function () {
            ...
            /*
            self.mlc.connections : 좌변이 연결된 우변의 idx값 목록
            ...
            */
            ...
            //console.log('좌변이 연결된 우변의 idx값 목록: ', self.mlc.connections);
            ...
            if (self.mlc.com() > 0) {
                ...
                // console.log(lineCon_ans, knAnsTotal, self.mlc.com());
                    ...
                    if (rightItem && !conn_x) {
                        ...
                        if (self.chkConnAns) {
                            ...
                            if (self.se52_1_0500_06) {
                                ...
                                const ltoRLine = $(`line[li="${lIdx}"][ri="${rIdx}"]`);
                                const rtoLLine = $(`line[li="${rIdx}"][ri="${lIdx}"]`);
                        ...
                        let removeDup = false;  // 기존의 정답과 연결된 선을 삭제했는지 여부
                        var lineDup = $(self.svg).find('line[li = "' + curIndex + '"][ri = "' + self.rightItems.indexOf(rightItem) + '"]');
                        if (lineDup.length > 0) {

                            // <line/>참조를 배열에서 삭제
                            if (self.lines.indexOf(lineDup[0]) > -1) {
                                self.lines.splice(self.lines.indexOf(lineDup[0]), 1);
                            }
                        ...
                        // console.log('(L → R) lines: ', self.lines);

                        // 1개만 연결가능 할 시....
                        if (self.Mode === 'single') {
                            // 현재 연결선을 제외한, ri와 연결된선 모두 삭제
                            $(self.svg).find('line').each(function () {
                                if (parseInt($(this).attr('li'), 10) === curIndex && $(this)[0] != line) {
            ...
            $(rightItem).off(downEvent).on(downEvent, function (e) {
                ...
                $(document).off(upEvent).on(upEvent, function (e) {
                    ...
                    if (leftItem && !conn_x) {
                        ...
                        let removeDup = false;  // 기존의 정답과 연결된 선을 삭제했는지 여부
                        var lineDup = $(self.svg).find('line[li = "' + self.leftItems.indexOf(leftItem) + '"][ri = "' + curIndex + '"]');
                        if (lineDup.length > 0) {
                        ...
                        // console.log('(R → L) lines: ', self.lines);

                        // 1개만 연결가능 할 시....
                        if (self.Mode == 'single') {
                            // 현재 연결선을 제외한, ri와 연결된선 모두 삭제
                            $(self.svg).find('line').each(function () {
                                if (parseInt($(this).attr('li'), 10) === self.leftItems.indexOf(leftItem) && $(this)[0] != line) {
